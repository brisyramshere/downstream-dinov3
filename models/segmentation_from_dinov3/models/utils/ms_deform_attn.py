...
try:
    # Try to import from the local ops directory first
    import sys
    import os
    ops_path = os.path.join(os.path.dirname(__file__), 'ops')
    if ops_path not in sys.path:
        sys.path.insert(0, ops_path)
    import MultiScaleDeformableAttention as MSDA
except ImportError:
    try:
        # Fallback: try absolute import from ops subdirectory
        from .ops import MultiScaleDeformableAttention as MSDA
    except ImportError:
